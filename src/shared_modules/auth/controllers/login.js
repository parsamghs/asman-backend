...
<<<<<<< HEAD:controllers/authcontrollers/login.js
    if (user.role !== 'ادمین') {
      if (!user.dealer_id) {
...
=======
    const dealersRes = await pool.query(
      `SELECT d.id AS dealer_id, d.dealer_name, d.dealer_code, d.category
    ...
    if (dealersRes.rowCount === 0) {
      return res.status(403).json({ message: 'این کاربر به هیچ نمایندگی‌ای متصل نیست.' });
    }
    ...
    return res.json({
      token: finalToken,
>>>>>>> change-dealers-format:src/shared_modules/auth/controllers/login.js
      name: user.name,
